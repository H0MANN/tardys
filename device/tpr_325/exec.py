...
def read_and_save(tpr):  # 'tpr' is necessary to use this decorator...
    ...
    ## Reads each channel.
    temp1 = tpr.read_temp(1).decode()
    print(temp1)
    temp2 = tpr.read_temp(2).decode()

    setp1 = tpr.read_setp(1).decode()
    setp2 = tpr.read_setp(2).decode()

    hout1 = tpr.read_hout(1).decode()
    hout2 = tpr.read_hout(2).decode()

    mout1 = tpr.mout(1).decode()
    mout2 = tpr.mout(2).decode()

    print(tpr.ramp(1))
    ramp1, ramp_rate1 = tpr.ramp(1).decode().split(",")
    ramp2, ramp_rate2 = tpr.ramp(2).decode().split(",")

    pid1 = tpr.pid(1).decode().split(",")
    pid2 = tpr.pid(2).decode().split(",")

    hrange = tpr.hrange().decode()
...
if __name__ == "__main__":
    from taoics.tardys.device.tpr_325 import build_tpr
