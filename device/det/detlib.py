...
class Detlib():
    ...
    def write_image(self, rsltdir, frame_id, image, header):
        """
        ...
        hdulist.writeto(rsltpath, overwrite = True)
    ...
    def get_hot_pixel(self, weights):
        ...
        """
        hot_pixel_map = np.where((np.sum(weights, axis=0)==0) | (np.sum(weights, axis=0)==1), 1, 0)
        return hot_pixel_map

    def get_messia6_header(self, rawdir):
        header = fits.getheader(self.list_filename(rawdir)[-1])
        return header 

    def add_header(self, header, keys):
        dict = self.common.select_status_dict(keys)
    ...
    def get_fowler(self, rawdir, reset_sub):
        """
        ...
        if reset_sub:
            print("Reset frame is already subtracted.")
            stacked_sub_arr = stacked_arr
        else:
            print("Subtract reset frame.")
    ...
    def get_sur(self, rawdir, exp_time, reset_sub):
        """
        Args:
            rawdir(str) : The path of the directory where the raw data is.
            rsltdir(str) : The path of the directory where the resulting data will be.
    ...
    def create_fits(self, rawdir, rsltdir, frame_id, exp_time, sample_mode):
        ...
        reset_sub, ref_sub = map(self.common.true_or_false, self.common.select_status_values(["RST_SUB", "REF_SUB"]))

        #header written by messia6
        header = self.get_messia6_header(rawdir)
        #keys = ["SMPLMODE", "N_OUT", "RST_SUB", "REF_SUB", "CODING", "EXP_TIME"]
        #header = self.add_header(header, keys)

        if sample_mode == "FOWLER":
            frame = self.get_fowler(rawdir, reset_sub)
        else:
            frame = self.get_sur(rawdir, exp_time, reset_sub)
        if ref_sub:
            print("Reference frame is already subtracted.")
            rslt_frame = frame
        else:
            print("Subtract reference frame")
            rslt_frame = self.ref_pix_sub(frame)
        self.write_image(rsltdir, frame_id, rslt_frame, header)
